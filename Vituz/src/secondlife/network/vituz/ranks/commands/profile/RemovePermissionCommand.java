...
public class RemovePermissionCommand extends BaseCommand {
    ...
    public RemovePermissionCommand(Vituz plugin) {
        super(plugin);

        this.command = "removepermission";
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length != 2) {
            sender.sendMessage(Color.translate("&cUsage: /removepermission <player> <permission>"));
            return;
        }
        ...
        if(!profile.getPermissions().contains(permission)) {
            sender.sendMessage(Color.translate("&cPlayer named '" + args[0] + "' doesn't have permission node '" + permission + "'."));
            return;
        }
        ...
        if(target == null) {
            ...
            object.addProperty("action", RankSubscriberAction.DELETE_PLAYER_PERMISSION.name());
        ...
        profile.save();
        sender.sendMessage(Color.translate("&aPermission '" + permission + "' successfully removed from player named '" + args[0] + "'."));
    }
}
