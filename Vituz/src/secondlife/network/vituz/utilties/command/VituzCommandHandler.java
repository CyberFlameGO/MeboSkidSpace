package secondlife.network.vituz.utilties.command;
...
import secondlife.network.vituz.utilties.command.param.ParameterData;
import secondlife.network.vituz.utilties.command.param.ParameterType;
import secondlife.network.vituz.utilties.command.param.defaults.*;
...
public final class VituzCommandHandler implements Listener {
    ...
    public static void hook() {
        ...
        Vituz.getInstance().getServer().getPluginManager().registerEvents(new VituzCommandHandler(), Vituz.getInstance());
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    ...
                    Field commandMapField = Vituz.getInstance().getServer().getClass().getDeclaredField("commandMap");
                    commandMapField.setAccessible(true);

                    Object oldCommandMap = commandMapField.get(Vituz.getInstance().getServer());
                    VituzCommandMap newCommandMap = new VituzCommandMap(Vituz.getInstance().getServer());
                    ...
                    commandMapField.set(Vituz.getInstance().getServer(), newCommandMap);
                } catch (Exception e) {
        ...
        }.runTaskLater(Vituz.getInstance(), 5L);
    ...
    protected static void registerMethod(Method method) {
        ...
        for (int parameterIndex = 1; parameterIndex < method.getParameterTypes().length; parameterIndex++) {
            ...
            if (parameterAnnotation != null) {
                ...
                Vituz.getInstance().getLogger().warning("Method '" + method.getName() + "' has a parameter without a @Parameter annotation.");
                return;
            }
        }
    ...
    public static String[] getParameters(Player player) {
        return VituzCommandMap.parameters.get(player.getUniqueId());
    }
    ...
    public void onCommandPreProcess(PlayerCommandPreprocessEvent event) {
        ...
        VituzCommandMap.parameters.put(event.getPlayer().getUniqueId(), command.split(" "));
