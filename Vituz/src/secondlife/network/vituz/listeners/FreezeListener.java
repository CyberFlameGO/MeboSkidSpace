...
public class FreezeListener implements Listener {
	...
	private String[] message = {
			"",
			"",
			"&4&lPlayer %PLAYER% quit while frozen!",
			"",
			"",
	};
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		if(!PlayerData.getByName(event.getEntity().getName()).isFrozen()) return;
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		Stream.of(message).forEach(message ->
				Msg.sendMessage(Color.translate(message.replace("%PLAYER%", event.getPlayer().getName())), Permission.STAFF_PERMISSION)
		);
	}
}
