...
public class DeathLookup {
    ...
    public long getDeathsCount() {
        return Vituz.getInstance().getDatabaseManager().getUhcDeaths().count(eq("killed", deathData.getName())) + deathData.getFights().size();
    }
    ...
    public List<ProfileFight> getDeaths(int page) {
        ...
        MongoCursor cursor = Vituz.getInstance().getDatabaseManager().getUhcDeaths().find(eq("killed", deathData.getName().toString())).skip((page - 1) * PAGE_SIZE).limit(PAGE_SIZE - toLimit).sort(descending("occurred_at")).iterator();
