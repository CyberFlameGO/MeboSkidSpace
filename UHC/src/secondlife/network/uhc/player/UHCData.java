package secondlife.network.uhc.player;
...
public class UHCData {
	...
	public void save() {
		...
		Vituz.getInstance().getDatabaseManager().getUhcProfiles().replaceOne(Filters.eq("name", this.name.toLowerCase()), document, new UpdateOptions().upsert(true));

		loaded = false;
	}

	public void load() {
		Document document = (Document) Vituz.getInstance().getDatabaseManager().getUhcProfiles().find(Filters.eq("name", this.name.toLowerCase())).first();
