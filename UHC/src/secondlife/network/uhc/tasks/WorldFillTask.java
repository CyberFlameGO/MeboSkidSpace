...
import secondlife.network.uhc.border.worldborder.WorldFileData;
import secondlife.network.uhc.utilties.events.WorldBorderFillFinishedEvent;
import secondlife.network.uhc.utilties.events.WorldBorderFillStartEvent;
...
public class WorldFillTask implements Runnable
{
	...
	public void finish()
	{
		...
		sendMessage("task completed for world \"" + refWorld() + "\".");
		this.stop();
	}
	...
	private void reportProgress()
	{
		...
		Bukkit.broadcastMessage(Color.translate("&d" + reportNum + " &emore chunk processed (&d" + (reportTotal + reportNum) + " &etotal, ~ &d" + Config.coord.format(perc) + "&e%)"));
