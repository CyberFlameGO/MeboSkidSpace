...
import secondlife.network.uhc.listeners.ChatListener;
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(args.length == 0) {
			...
			if(ChatListener.chat.contains(player.getUniqueId())) {
				...
				player.sendMessage(Color.translate("&eYou have &cDisabled &dTeam Chat&7."));
			} else {
				ChatListener.chat.add(player.getUniqueId());
				
				player.sendMessage(Color.translate("&eYou have &a&lEnabled &dTeam Chat&7."));
			}
			...
			party.broadcast("&d" + player.getName() + " &eCoords -> x&d- &d" + x + "&7, y&d- &d" + y + "&7, z&d- &d" + z + "&7.");
			...
			if(party.isDamageMembers()) {
				...
				party.broadcast("&d" + party.getOwner().getName() + " &ehas &cdisabled&e party damage.");
			} else {
				party.setDamageMembers(true);
				
				party.broadcast("&d" + party.getOwner().getName() + " &ehas &aenabled&e party damage.");
			}
		}
	}
