...
import secondlife.network.uhc.tasks.PracticeTask;
...
public class UHCPracticeCommand extends BaseCommand {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(args[0].equalsIgnoreCase("join")) {
			...
			player.sendMessage(Color.translate("&eYou have &ejoined &dPractice&e."));
			...
			new BukkitRunnable() {
				public void run() {
					try {
						...
						player.sendMessage(Color.translate("&eYou have &ereceived &dKit&e."));
					} catch (IOException e) {
						e.printStackTrace();
					}
				}
			...
			if(player.hasPermission(Permission.OP_PERMISSION)) {
				...
				player.sendMessage(Color.translate("&eYou have set &dUHCPractice &eitems."));
			}
			...
			if(player.hasPermission(Permission.OP_PERMISSION)) {
				...
				Msg.sendMessage("&dPractice&e has been &aEnabled&e by &d" + sender.getName() + "&e.");
				Msg.sendMessage("&eYou can join this by typing &d/uhcpractice join&e.");
			...
			if(player.hasPermission(Permission.OP_PERMISSION)) {
				...
				Msg.sendMessage("&dPractice&e has been &cDisabled&e by &d" + sender.getName() + "&e.");
			...
			if(sender.hasPermission(Permission.OP_PERMISSION)) {
				...
				Msg.sendMessage("&dPractice&e slots have been set to &d" + slots + "&e by &d" + sender.getName() + "&e.");
			}
		}
