...
import secondlife.network.uhc.tasks.WorldTrimTask;




public class WBTrimCommand extends WBCmd
{
	public WBTrimCommand()
	{
	...
	public void execute(CommandSender sender, Player player, List<String> params, String worldName)
	{
		...
		{	// command confirmed, go ahead with it
			...
			{
				int task = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(UHC.getInstance(), Config.trimTask, ticks, ticks);
	...
	private boolean makeSureTrimIsRunning(CommandSender sender)
	{
		if (Config.trimTask != null && Config.trimTask.valid())
			return true;
		sendErrorAndHelp(sender, "The world map trimming task isn't currently running.");
		return false;
	}
}
