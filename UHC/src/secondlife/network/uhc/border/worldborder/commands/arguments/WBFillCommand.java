package secondlife.network.uhc.border.worldborder.commands.arguments;
...
import secondlife.network.uhc.border.worldborder.CoordXZ;
import secondlife.network.uhc.tasks.WorldFillTask;



public class WBFillCommand extends WBCmd
{
	public WBFillCommand()
	{
	...
	public void execute(CommandSender sender, Player player, List<String> params, String worldName)
	{
		...
		{	// command confirmed, go ahead with it
			...
			{
				sendErrorAndHelp(sender, "You must first use this command without confirming.");
				return;
			}
...
/*	*/		Config.log("world: " + fillWorld + "  padding: " + fillPadding + "  repeats: " + repeats + "  ticks: " + ticks);			
			...
			{
				int task = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(UHC.getInstance(), Config.fillTask, ticks, ticks);
	...
	private boolean makeSureFillIsRunning(CommandSender sender)
	{
		if (Config.fillTask != null && Config.fillTask.valid())
			return true;
		sendErrorAndHelp(sender, "The world map generation task isn't currently running.");
		return false;
	}
}
