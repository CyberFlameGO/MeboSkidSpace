...
import secondlife.network.uhc.border.InvisibleFix;
...
public class BorderManager extends Manager {
            ...
            public void run() {
                ...
                if (i >= 1) {
					...
					/*if(border == 3000) {
						...
						shrinkBorder(2500, this);

						BorderTimeTask.setSeconds();

						Bukkit.broadcastMessage(Color.translate("&eBorder has shrunk to &d" + border + "&e."));
					} else */if(border == 2500) {
						new Border(w, 2000);

						InvisibleFix.fixPlayer(player);

						shrinkBorder(2000, this);
    ...
    public static void startBorderShrink() {
        Bukkit.broadcastMessage(Color.translate("&eBorder will shrink every &d" + OptionManager.getByNameAndTranslate("Border Shrink Interval") + " &eminutes, by &d500 &eblocks, until &d25&ex&d25&e."));
