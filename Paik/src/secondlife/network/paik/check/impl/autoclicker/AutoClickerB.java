...
public class AutoClickerB extends PacketCheck {
	...
	public AutoClickerB(Paik plugin, PlayerData playerData) {
	...
	public void handleCheck(Player player, Packet packet) {
		if(packet instanceof PacketPlayInArmAnimation && !this.playerData.isDigging() &&
		    ...
		    this.playerData.getLastMovePacket() != null && System.currentTimeMillis() - this.playerData.getLastMovePacket().getTimestamp() < 110L &&
			...
			if(this.flyingCount < 10) {
				...
				if(this.flyingCount > 3) {
					++this.outliers;
				} else if(this.flyingCount == 0) return;
				...
				if(++this.clicks == 1000) {
					...
					if(this.outliers <= 7) {
						if((vl += 1.4) >= 4.0) {
							this.alert(PlayerAlertEvent.AlertType.DEVELOPMENT, player, String.format("O %s. VL %.2f.", this.outliers, vl), false);
						}
					} else {
						vl -= 0.8;
					}
