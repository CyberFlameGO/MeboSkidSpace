...
public class VelocityC extends PositionCheck {
	...
	public VelocityC(Paik plugin, PlayerData playerData) {
	...
	public void handleCheck(Player player, PositionUpdate update) {
		...
		if(this.playerData.getVelocityY() > 0.0 && this.playerData.isWasOnGround() &&
			...
			if(ratio < 0.62) {
				if((vl += 1.1) >= 8.0 && this.alert(PlayerAlertEvent.AlertType.RELEASE, player, String.format(
						"P %s. VL %.2f.", Math.round(ratio * 100.0), vl), false) &&
