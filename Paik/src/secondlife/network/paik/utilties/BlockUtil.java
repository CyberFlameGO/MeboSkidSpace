...
public class BlockUtil {
    ...
    public static Material[] items = new Material[] {
            Material.DIAMOND_HOE, Material.GOLD_HOE, Material.IRON_HOE,
            Material.STONE_HOE, Material.WOOD_HOE, Material.STONE_SPADE,
            Material.GOLD_SPADE, Material.DIAMOND_SPADE, Material.IRON_SPADE,
            Material.WOOD_SPADE, Material.AIR };

    public static boolean isOnStairs(Location location, int down) {
        return isUnderBlock(location, blockStairsSet, down);
    }

    public static boolean checkMaterial(Player player) {
    ...
    private static boolean isUnderBlock(Location location, Set<Byte> itemIDs, int down) {
        ...
        if(itemIDs.contains((byte)world.getBlockAt(blockX, blockY, blockZ).getTypeId())) return true;
        
        if(fracX < 0.3) {
            if(itemIDs.contains((byte)world.getBlockAt(blockX - 1, blockY, blockZ).getTypeId())) return true;
    ...
    public static boolean isOnGround(Location location, int down) {
        ...
        if(!blockSolidPassSet.contains((byte)world.getBlockAt(blockX, blockY, blockZ).getTypeId())) return true;
        
        if(fracX < 0.3) {
            if(!blockSolidPassSet.contains((byte)world.getBlockAt(blockX - 1, blockY, blockZ).getTypeId())) return true;
            
            if(fracZ < 0.3) {
                if(!blockSolidPassSet.contains((byte)world.getBlockAt(blockX - 1, blockY, blockZ - 1).getTypeId())) return true;
                if(!blockSolidPassSet.contains((byte)world.getBlockAt(blockX, blockY, blockZ - 1).getTypeId())) return true;
                if(!blockSolidPassSet.contains((byte)world.getBlockAt(blockX + 1, blockY, blockZ - 1).getTypeId())) return true;
    ...
    static {
        blockSolidPassSet = new HashSet<Byte>();
        blockStairsSet = new HashSet<Byte>();
        blockLiquidsSet = new HashSet<Byte>();
        blockWebsSet = new HashSet<Byte>();
        blockIceSet = new HashSet<Byte>();
        blockCarpetSet = new HashSet<Byte>();
